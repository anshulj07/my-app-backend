...
export async function GET(req: Request) {
  ...
  try {
    ...
    const [createdEventsRaw, createdServicesRaw] = await Promise.all([
      db.collection("events").find({ creatorClerkId: clerkUserId }).sort({ createdAt: -1 }).limit(500).toArray(),
      db.collection("services").find({ creatorClerkId: clerkUserId }).sort({ createdAt: -1 }).limit(500).toArray()
    ]);
    
    // Merge and sort
    const allCreatedRaw = [...createdEventsRaw, ...createdServicesRaw].sort((a: any, b: any) => {
